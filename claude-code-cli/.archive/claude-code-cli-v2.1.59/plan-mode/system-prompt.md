...
> This mode sent 7 requests total: 6 opus (main) + 1 haiku (overhead). The main system prompt below is **identical** to agent mode — only the billing header hash varies per request. The overhead prompt is also identical to agent mode. Mode-specific behavior is controlled through `<system-reminder>` tags injected into conversation turns at runtime, not through the system prompt itself.

## Main Prompt — claude-opus-4-6 | has tools

**Words:** 2,345 | **Characters:** 15,107
...
**Purpose:** File path extraction from command outputs
**Words:** 179 | **Characters:** 1,181
