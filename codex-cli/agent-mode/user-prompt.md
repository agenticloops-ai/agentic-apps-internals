...
> Codex CLI sends the user message as 3 separate user turns — AGENTS.md skill instructions, environment context (`cwd` + `shell`), and the task. Unlike Claude Code (which uses `<system-reminder>` blocks), Codex uses dedicated XML tags (`<INSTRUCTIONS>`, `<environment_context>`) across distinct conversation turns.

The user message is split into three separate user turns:

1. **AGENTS.md instructions** — skill discovery and usage rules
2. **Environment context** — working directory and shell info
